...
class circle(object):
    def __init__(self,x,y,r,color):
        ...
        self.acce=acceleratespeed
        self.direction="none"
    def follow(self,x,y,accelerate,ishead):
        ...
        if(x==self.x and y==self.y):
            ...
            if(x<self.x):
                self.angle+=math.pi
        if(math.pi/32<self.angle-prev_angle<=math.pi):
            self.angle=prev_angle+math.pi/32
        if(math.pi<self.angle-prev_angle<math.pi/32*63):
            self.angle=prev_angle-math.pi/32
        if(-math.pi<=self.angle-prev_angle<-math.pi/32):
            self.angle=prev_angle-math.pi/32
        if(-math.pi/32*63<self.angle-prev_angle<-math.pi):
            self.angle=prev_angle+math.pi/32
        #get speed
        speed=self.speed
        if(accelerate):
            speed=self.acce
        if(((x-self.x)**2+(y-self.y)**2)<(speed**2) or not ishead):
            self.x=x
            self.y=y
        else:
...
class body(object):
    ...
    def follow(self,x,y,accelerate):#accelerate is a bool flag
        if accelerate and self.energy<=0:
            ...
            self.energy-=1
            self.update()
        if(self.bodypart[0].follow(x,y,accelerate,True)=="die"):
            return "die"
        for i in range(1,self.num):            
            self.bodypart[i].follow(self.bodypart[i-1].x-math.cos(self.bodypart[i-1].angle)*5,self.bodypart[i-1].y-math.sin(self.bodypart[i-1].angle)*5,accelerate,False)
        return "go"
